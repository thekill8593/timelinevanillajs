class Timeline {
    ...
    moveTimeline(dir = "left") {
        ...
        if ((currentMargin === 0 && dir === "right" )|| (Math.abs(currentMargin) >= (this.timelineWidth - this.timelineContainerWidth) && dir === "left")) {
            this.isAnimating = false;
            return;
        }
        ...
        const val = dir === "left" ? -(numberOfItemsToShow*this.itemWidth) : (numberOfItemsToShow*this.itemWidth);
    ...
    drawButtons () {
        if (this.itemsShowed === this.options.itemsPerPage) {
            this.btnGoRight.style.display = 'none';
        } else {
            this.btnGoRight.style.display = 'block';
        }

        if (this.itemsShowed === this.items.length) {
            this.btnGoLeft.style.display = 'none';
        } else {
            this.btnGoLeft.style.display = 'block';
        }
    }
